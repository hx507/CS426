...
@str.0 = internal constant [22 x i8] c"shadow-attr-formal.cl\00"
...
ok.0:
	...
	%tmp.10 = bitcast %Main* %tmp.9 to %Object*
	ret %Object* %tmp.10
...
entry:
	%tmp.11 = alloca %Main*
	store %Main* %self, %Main** %tmp.11
	%tmp.12 = load %Main*, %Main** %tmp.11
...
ok.1:
	...
	%tmp.16 = getelementptr %_Main_vtable, %_Main_vtable* %tmp.15, i32 0, i32 11
	%tmp.17 = load %Object* (%Main*,%String*) *, %Object* (%Main*,%String*) ** %tmp.16
	%tmp.18 = call %Object*(%Main*, %String* ) %tmp.17( %Main* %tmp.12, %String* @String.2 )
	%tmp.19 = load %Main*, %Main** %tmp.11
...
ok.2:
	...
	%tmp.25 = call %Main*(%Main*, %String* ) %tmp.24( %Main* %tmp.19, %String* @String.3 )
...
okay:
	...
	%tmp.34 = call %String* @String_new(  )
	store %String* %tmp.34, %String** %tmp.33
	%tmp.35 = getelementptr %Main, %Main* %tmp.31, i32 0, i32 1
	store %String* @String.1, %String** %tmp.35
	ret %Main* %tmp.31
