...
@str.0 = internal constant [15 x i8] c"dispatch1_o.cl\00"
...
define i32 @Main.niam(%Main* %self) {
...
entry:
	...
	%tmp.2 = load %Main*, %Main** %tmp.1
	%tmp.3 = icmp eq %Main* %tmp.2, null
	br i1 %tmp.3, label %abort, label %ok.1

ok.1:
	%tmp.4 = getelementptr %Main, %Main* %tmp.2, i32 0, i32 0
	%tmp.5 = load %_Main_vtable*, %_Main_vtable** %tmp.4
	%tmp.6 = getelementptr %_Main_vtable, %_Main_vtable* %tmp.5, i32 0, i32 11
	%tmp.7 = load i32 (%Main*) *, i32 (%Main*) ** %tmp.6
	%tmp.8 = call i32(%Main* ) %tmp.7( %Main* %tmp.2 )
	%tmp.9 = load %Main*, %Main** %tmp.1
...
ok.0:
	...
	%tmp.13 = getelementptr %_Main_vtable, %_Main_vtable* %tmp.12, i32 0, i32 8
