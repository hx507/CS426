...
@str.0 = internal constant [12 x i8] c"selftype.cl\00"
...
entry:
	%tmp.2 = alloca %A*
	%tmp.3 = getelementptr %_A_vtable, %_A_vtable* @_A_vtable_prototype, i32 0, i32 1
	%tmp.4 = load i32, i32* %tmp.3
	%tmp.5 = call i8*(i32 ) @malloc( i32 %tmp.4 )
	%tmp.6 = bitcast i8* %tmp.5 to %A*
	%malloc.null = icmp eq %A* %tmp.6, null
	br i1 %malloc.null, label %abort, label %okay

okay:
	%tmp.7 = getelementptr %A, %A* %tmp.6, i32 0, i32 0
	store %_A_vtable* @_A_vtable_prototype, %_A_vtable** %tmp.7
	store %A* %tmp.6, %A** %tmp.2
	ret %A* %tmp.6
...
ok.1:
	...
	%tmp.6 = load %B* (%B*) *, %B* (%B*) ** %tmp.5
	%tmp.7 = call %B*(%B* ) %tmp.6( %B* %tmp.1 )
	%tmp.8 = icmp eq %B* %tmp.7, null
	br i1 %tmp.8, label %abort, label %ok.0

ok.0:
	%tmp.9 = getelementptr %B, %B* %tmp.7, i32 0, i32 0
	%tmp.10 = load %_B_vtable*, %_B_vtable** %tmp.9
	%tmp.11 = getelementptr %_B_vtable, %_B_vtable* %tmp.10, i32 0, i32 5
	%tmp.12 = load %String* (%B*) *, %String* (%B*) ** %tmp.11
	%tmp.13 = call %String*(%B* ) %tmp.12( %B* %tmp.7 )
	ret %String* %tmp.13
