...
@str.0 = internal constant [17 x i8] c"letinittrap_o.cl\00"
...
entry:
	...
	store i1 %tmp.4, i1* %tmp.2
	%tmp.5 = load i1, i1* %tmp.2
...
ok.0:
	...
	%tmp.12 = call %Main*(%Main*, i32 ) %tmp.11( %Main* %tmp.6, i32 1 )
	store %Main* %tmp.12, %Main** %tmp.3
	br label %end.0
...
ok.1:
	...
	%tmp.19 = call %Main*(%Main*, i32 ) %tmp.18( %Main* %tmp.13, i32 0 )
	store %Main* %tmp.19, %Main** %tmp.3
	br label %end.0

end.0:
	%tmp.20 = load %Main*, %Main** %tmp.3
	%tmp.21 = load %Main*, %Main** %tmp.0
	%tmp.22 = bitcast %Main* %tmp.21 to %Object*
	ret %Object* %tmp.22
