...
@str.0 = internal constant [21 x i8] c"shadow-formal-let.cl\00"
...
entry:
	...
	store %String* @String.1, %String** %tmp.2
	%tmp.3 = load %String*, %String** %tmp.2
	%tmp.4 = load %Main*, %Main** %tmp.0
	%tmp.5 = icmp eq %Main* %tmp.4, null
	br i1 %tmp.5, label %abort, label %ok.0

ok.0:
	%tmp.6 = getelementptr %Main, %Main* %tmp.4, i32 0, i32 0
	%tmp.7 = load %_Main_vtable*, %_Main_vtable** %tmp.6
	%tmp.8 = getelementptr %_Main_vtable, %_Main_vtable* %tmp.7, i32 0, i32 7
	%tmp.9 = load %Main* (%Main*,%String*) *, %Main* (%Main*,%String*) ** %tmp.8
	%tmp.10 = call %Main*(%Main*, %String* ) %tmp.9( %Main* %tmp.4, %String* %tmp.3 )
	%tmp.11 = bitcast %Main* %tmp.10 to %Object*
	ret %Object* %tmp.11
...
ok.1:
	...
	%tmp.17 = getelementptr %_Main_vtable, %_Main_vtable* %tmp.16, i32 0, i32 11
	%tmp.18 = load %Object* (%Main*,%String*) *, %Object* (%Main*,%String*) ** %tmp.17
	%tmp.19 = call %Object*(%Main*, %String* ) %tmp.18( %Main* %tmp.13, %String* @String.2 )
	%tmp.20 = load %Main*, %Main** %tmp.12
	%tmp.21 = icmp eq %Main* %tmp.20, null
	br i1 %tmp.21, label %abort, label %ok.2

ok.2:
	%tmp.22 = getelementptr %Main, %Main* %tmp.20, i32 0, i32 0
	%tmp.23 = load %_Main_vtable*, %_Main_vtable** %tmp.22
	%tmp.24 = getelementptr %_Main_vtable, %_Main_vtable* %tmp.23, i32 0, i32 7
	%tmp.25 = load %Main* (%Main*,%String*) *, %Main* (%Main*,%String*) ** %tmp.24
	%tmp.26 = call %Main*(%Main*, %String* ) %tmp.25( %Main* %tmp.20, %String* @String.3 )
