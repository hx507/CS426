...
@str.0 = internal constant [20 x i8] c"shadow-attr-case.cl\00"
...
entry:
	...
	%tmp.2 = icmp eq %String* @String.2, null
	br i1 %tmp.2, label %abort, label %ok.0

ok.0:
	%tmp.3 = getelementptr %String, %String* @String.2, i32 0, i32 0
	%tmp.4 = load %_String_vtable*, %_String_vtable** %tmp.3
	%tmp.5 = getelementptr %_String_vtable, %_String_vtable* %tmp.4, i32 0, i32 0
...
case.exit.0:
	%tmp.15 = load %Main*, %Main** %tmp.1
	%tmp.16 = load %Main*, %Main** %tmp.0
	%tmp.17 = icmp eq %Main* %tmp.16, null
	br i1 %tmp.17, label %abort, label %ok.2
...
ok.2:
	...
	%tmp.22 = call %Main*(%Main*, %String* ) %tmp.21( %Main* %tmp.16, %String* @String.3 )
...
okay:
	...
	%tmp.30 = getelementptr %Main, %Main* %tmp.28, i32 0, i32 1
	%tmp.31 = call %String* @String_new(  )
	store %String* %tmp.31, %String** %tmp.30
	%tmp.32 = getelementptr %Main, %Main* %tmp.28, i32 0, i32 1
	store %String* @String.1, %String** %tmp.32
	ret %Main* %tmp.28
