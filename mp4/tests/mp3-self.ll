...
@str.0 = internal constant [8 x i8] c"self.cl\00"
...
entry:
	...
	%main.retval = call %Alpha*(%Main*) @Main.main( %Main* %main.obj )
	ret i32 0
}

define %Alpha* @Alpha.getSelf(%Alpha* %self) {
...
entry:
	%tmp.2 = alloca %Alpha*
	%tmp.3 = getelementptr %_Alpha_vtable, %_Alpha_vtable* @_Alpha_vtable_prototype, i32 0, i32 1
	%tmp.4 = load i32, i32* %tmp.3
	%tmp.5 = call i8*(i32 ) @malloc( i32 %tmp.4 )
	%tmp.6 = bitcast i8* %tmp.5 to %Alpha*
	%malloc.null = icmp eq %Alpha* %tmp.6, null
	br i1 %malloc.null, label %abort, label %okay

okay:
	%tmp.7 = getelementptr %Alpha, %Alpha* %tmp.6, i32 0, i32 0
	store %_Alpha_vtable* @_Alpha_vtable_prototype, %_Alpha_vtable** %tmp.7
	store %Alpha* %tmp.6, %Alpha** %tmp.2
	ret %Alpha* %tmp.6
...
define %Alpha* @Main.main(%Main* %self) {
...
ok.0:
	...
	%tmp.6 = load %Alpha* (%Alpha*) *, %Alpha* (%Alpha*) ** %tmp.5
	%tmp.7 = call %Alpha*(%Alpha* ) %tmp.6( %Alpha* %tmp.1 )
	ret %Alpha* %tmp.7
